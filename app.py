...
if st.sidebar.button("🔍 위험도 예측하기"):
   ...
   # ── 지도 히트맵 ──
    ...
    if shap:
        ...
        st.caption("각 항목이 위험도 점수에 얼마나 기여했는지 보여줍니다.")

        shap_df = pd.DataFrame({
            "항목": list(shap.keys()),
            "영향도": list(shap.values())
        }).sort_values("영향도", ascending=True).reset_index(drop=True)

        # Waterfall 계산
        base = score - shap_df["영향도"].sum()
        running = base
        starts, widths, colors = [], [], []
        for val in shap_df["영향도"]:
        ...
        bars = ax.barh(shap_df["항목"], widths, left=starts, color=colors)

        # 기준선 (base value)
        ax.axvline(base, color="gray", linestyle="--", linewidth=1, label=f"기준값: {base:.1f}")
        # 최종 점수선
        ax.axvline(score, color="black", linestyle="-", linewidth=1.5, label=f"위험지수: {score}")

        for i, (s, w) in enumerate(zip(starts, widths)):
            ax.text(s + w + 0.3, i, f"{w:+.1f}", va='center', fontsize=9)

        ax.set_xlabel("위험지수 기여도")
        ax.set_title(f"{dong_name} / {target} Waterfall Plot")
